...
def dir_serv_perc(df):
    df_perc = df[(df['Serving']=='Serving')&(df['isAce']==False)].groupby(['Service','dir_srv']).agg({'count':'sum'})
    df_perc['perc_of_total'] = 100 * df_perc['count'] / df_perc.groupby('Service')['count'].transform('sum')
    df_perc = df_perc.reset_index()
    df_perc['perc_of_total'] = df_perc['perc_of_total'].round(decimals=1)

    first_down = str(df_perc['perc_of_total'][(df_perc['Service']=='1st')&(df_perc['dir_srv']=='Down the T')].item())+'%'
    first_body = str(df_perc['perc_of_total'][(df_perc['Service']=='1st')&(df_perc['dir_srv']=='Body')].item())+'%'
    first_wide = str(df_perc['perc_of_total'][(df_perc['Service']=='1st')&(df_perc['dir_srv']=='Wide')].item())+'%'

    second_down = str(df_perc['perc_of_total'][(df_perc['Service']=='2nd')&(df_perc['dir_srv']=='Down the T')].item())+'%'
    second_body = str(df_perc['perc_of_total'][(df_perc['Service']=='2nd')&(df_perc['dir_srv']=='Body')].item())+'%'
    second_wide = str(df_perc['perc_of_total'][(df_perc['Service']=='2nd')&(df_perc['dir_srv']=='Wide')].item())+'%'


    return first_down, first_body, first_wide, second_down, second_body, second_wide
